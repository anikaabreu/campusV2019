window.PDK || (function (window) {
  ...
  var Api = {
    ...
    executeCorsRequest: function (a, b, c, d, e, f, g) {
      console.log('longasslist', a, b, c, d, e, f, g, QS.encode(d))
  ...
  include(PDK, {
    init: function (a) {
      ...
      // console.log('INITFIRESETCOOKIE', Cookie)
    }
  ...
  UI.Methods = {
    "auth.login": {
      ...
      transform: function (a) {
        ...
        console.log(b)
        if (b) return a.useRedirect = !window.postMessage || /(iPhone|iPod|iPad).*AppleWebKit(?!.*Version)/i.test(navigator.userAgent) || -1 !== navigator.userAgent.indexOf("MSIE") || navigator.appVersion.indexOf("Trident/") > 0, UI._defaultCb[a.id] = function (c) {
          var d = {};
          console.log('agafg', c)
          if (c && c.access_token) {
            var e = {
  ...
  var Cookie = {
    ...
    QS = {
      encode: function (a, b, c) {
        ...
        //  console.log('returna', d)
        let val;
        for (var e in a) 
        ( val = a[e], null !== val && "undefined" != typeof val && d.push(c(e) + "=" + c(val)));
        //  console.log('log', d.sort(), d.join(b))
 ...
 *  @param {Function} callback - function fired on completion
 */
export function myBoards(callback) {
  PDK.me('boards', {
    fields: 'id,name,image[small]'
  }, callback);
}
// };
